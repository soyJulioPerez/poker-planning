...
import { NewRoundRequest } from 'shared-contracts';
...
export async function handleNewRound(
  ...
  if (name !== meta.moderatorName) {
    await sendToConnection(apiEndpoint, connectionId, {
      type: 'error',
      message: 'Only the moderator can start a new round',
    });
    return;
  }
  ...
  await ddb.send(
    new UpdateCommand({
      ...
      UpdateExpression: 'SET roundPhase = :idle, revealResult = :null',
      ExpressionAttributeValues: { ':idle': 'idle', ':null': null },
    })
  );
